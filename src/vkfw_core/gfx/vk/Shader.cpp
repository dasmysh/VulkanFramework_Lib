...
namespace vkfw_core::gfx {
        ...
        m_strType{ "vertex" }
    {
        if (ends_with(m_shaderFilename, ".frag")) {
            ...
            m_type = vk::ShaderStageFlagBits::eRaygenNV;
            m_strType = "raygen";
        } else if (ends_with(m_shaderFilename, ".rint")) {
            m_type = vk::ShaderStageFlagBits::eIntersectionNV;
            m_strType = "intersection";
        } else if (ends_with(m_shaderFilename, ".rahit")) {
            m_type = vk::ShaderStageFlagBits::eAnyHitNV;
            m_strType = "anyhit";
        } else if (ends_with(m_shaderFilename, ".rchit")) {
            m_type = vk::ShaderStageFlagBits::eClosestHitNV;
            m_strType = "closesthit";
        } else if (ends_with(m_shaderFilename, ".rmiss")) {
            m_type = vk::ShaderStageFlagBits::eMissNV;
            m_strType = "miss";
        } else if (ends_with(m_shaderFilename, ".rcall")) {
            m_type = vk::ShaderStageFlagBits::eCallableNV;
            m_strType = "callable";
        }
    ...
    void Shader::LoadCompiledShaderFromFile()
    {
        ...
        m_shaderModule = GetDevice()->GetDevice().createShaderModuleUnique(moduleCreateInfo);
    }
}
