...
namespace viscom::sinks {
    ...
    class rotating_open_file_sink final : public spdlog::sinks::base_sink<Mutex>
    {
    public:
    ...
    inline std::shared_ptr<spdlog::logger> rotating_open_logger_mt(
        const std::string& logger_name, const spdlog::filename_t& filename, std::size_t max_files)
    {
        return Factory::template create<rotating_open_file_sink_mt>(logger_name, filename, max_files);
    }

    template<typename Factory = spdlog::synchronous_factory>
    inline std::shared_ptr<spdlog::logger> rotating_open_logger_st(
        const std::string& logger_name, const spdlog::filename_t& filename, std::size_t max_files)
    {
